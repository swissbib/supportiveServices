 ...
 * Time: 11:36:18
 */
public class AvailabilityRequest  extends HttpServlet{
    ...
    protected void doGet(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws ServletException, IOException {
        ...
        String[] barcode = httpServletRequest.getParameterValues("barcode");
        ...
        try {
            CirculationStateService circService = new CirculationStateService(AvailabilityRequest.libraryProperties,
                                                                                AvailabilityRequest.mapInstitutions,
                                                                               AvailabilityRequest.proxyServer);
            circResponse = circService.getCirculationStatus(sysnumber,barcode,idls,debug,language);
            ...
            if (jsonResonse != null && catchedErrorMessage == null) {
                ...
                //String tempResponse = StringEscapeUtils.escapeHtml(response);
                //out.println(StringEscapeUtils.escapeHtml(tempResponse));
    ...
    public void init(ServletConfig servletConfig) throws ServletException {
        ...
        AvailabilityRequest.proxyServer = servletConfig.getInitParameter("proxyAdress");
        ...
        try {
            ...
            AvailabilityRequest.libraryProperties = createLibraryProperties(in);

            in = servletConfig.getServletContext().getResourceAsStream(configXServerConfig);


            AvailabilityRequest.mapInstitutions = createXServerConfig(in);




        } catch (ParserConfigurationException pE) {
            availLog.error("AvailabilityRequest.init: ParserConfigurationException: ", pE);

        }catch (IOException ioE) {
            availLog.error("AvailabilityRequest.init: IOException: ", ioE);
        }catch (SAXException sE)  {
            availLog.error("AvailabilityRequest.init: SAXException: ", sE);
        }
    ...
    private HashMap<String,LibraryProperties> createLibraryProperties(InputStream in)
        ...
        for (int i = 0; i < libraries.getLength();i++) {
            ...
            LibraryProperties lp = new LibraryProperties(idsystem,urlsystem, systemtype,
                    anonymusUser,circQueryParameter,useProxy);
    ...
    private JSONObject createJsonResponse(CirculationStateResponse circResponse,
        ...
        for (CirculationStateItem item : circResponse.getItemList()) {
            ...
            jsonResponse.put(item.getIdentifierBarcode(),item.getItemStatusInfomation());
